...
export class PonudePonudjaciComponent implements OnInit {
  ...
  trackId(index: number, item: IPonudePonudjaci): number {
    return item.id!;
  }
  ...
  protected onSuccess(data: IPonudePonudjaci[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/ponude-ponudjaci'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
